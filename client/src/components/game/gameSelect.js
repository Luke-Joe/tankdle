    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const storedDayId = Number(localStorage.getItem("dayId"));
                if (storedDayId !== solutionTank.dayId) {
                    console.log("Stored", storedDayId, "does not match", solutionTank.dayId);
    ...
    if (!solutionTank || !isLocalStorageChecked) {
        return (
            <div className="inline-flex mx-auto">
                <div role="status">
                    <svg aria-hidden="true" className="inline w-6 h-6 text-wblack animate-spin dark:text-wblack fill-wyellow" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
